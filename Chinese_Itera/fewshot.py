...
# parser.add_argument("--model_name_or_path", default='/home/ubuntu/user_wsq/TKDE_balance/model')
parser.add_argument("--model_name_or_path", default='/home/ubuntu/user_lzl/chinese_itera/model')
...
def evaluate2(prompt_model, dataloader, desc):
    ...
    for step, inputs in enumerate(pbar):
        ...
        alllabels.extend(labels.cpu().tolist())
        allpreds.extend(torch.argmax(logits, dim=-1).cpu().tolist())
        f_w = open('hc3_finance_prelabel0.csv', 'w', encoding='gbk')
...
# prompt_model.load_state_dict(torch.load(f"./ckpts/hc3_finance_tem0.ckpt"))
tot_loss = 0 
log_loss = 0
best_val_acc = 0
for epoch in range(args.max_epochs):
    ...
    if val_acc>=best_val_acc:
        torch.save(prompt_model.state_dict(),f"./ckpts/Mixdata_tem0.ckpt")
        best_val_acc = val_acc
    print("Epoch {}, val_acc {}".format(epoch, val_acc), flush=True)

prompt_model.load_state_dict(torch.load(f"./ckpts/Mixdata_tem0.ckpt"))
# prompt_model = prompt_model.cuda()
